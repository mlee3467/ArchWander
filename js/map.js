...
function _makeStreetLayer() {
  // Priority: MapTiler vector tiles → Thunderforest raster → CartoDB Voyager raster
  if (MAPTILER_API_KEY && typeof L.maplibreGL === 'function') {
    return L.maplibreGL({
      style: 'https://api.maptiler.com/maps/' + MAPTILER_STYLE + '/style.json?key=' + MAPTILER_API_KEY,
